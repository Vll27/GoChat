...
function ChatPage() {
  ...
  // 1. CORREGIDO: Un solo useEffect para el resize y limpieza del store al desmontar
  useEffect(() => {
    ...
    return () => {
      ...
      // Limpia el usuario seleccionado al salir de la página de chat (ej. cerrar sesión)
      setSelectedUser(null);
    };
  }, [setSelectedUser]);

  // 1. Traemos el fondo definitivo de la aplicación de forma reactiva
  ...
  return (
    ...
    <div
      // 2. ELIMINAMOS "bg-black" de aquí para que deje de ser estático
      className="w-full h-screen absolute inset-0 overflow-hidden text-slate-200 transition-all duration-300"
      style={{
        // 3. Pasamos el color dinámico del store mediante style inline (prioridad absoluta)
      ...
      <div className="w-full h-full relative z-10">
        <div className="w-full h-full relative overflow-hidden flex items-center justify-center">
          ...
          <div
            ...
            `}
            // 💡 APLICAMOS COLOR REACTIVO DESDE EL STORE A LA BURBUJA IZQUIERDA
            style={{
              backgroundColor: sidebarBgColor ? `${sidebarBgColor}cc` : "rgba(30, 41, 59, 0.6)", // 'cc' añade un toque de transparencia alfa
              borderColor: appBorderColor || "rgba(51, 65, 85, 0.4)"
            }}
          >
            {/* Header del panel central */}
            {/* Usamos el color del borde para separar secciones discretamente */}
          ...
          <div
            ...
            `}
            // 💡 APLICAMOS COLOR REACTIVO AL CONTENEDOR DEL CHAT ABIERTO
            style={{
              backgroundColor: sidebarBgColor ? `${sidebarBgColor}99` : "rgba(30, 41, 59, 0.4)", // Un toque más traslúcido para el área de mensajes
          ...
          {!selectedUser && (
            <div className="absolute bottom-4 left-4 z-30 flex flex-col items-center group">
              ...
              {/* 2. CORREGIDO: Se unificaron las clases en un único atributo className */}
              <button
              ...
              >
                <svg 
                  ...
                  stroke="currentColor" 
                  className="w-6 h-6"
                  style={{ color: "var(--theme-primary)" }} // Hace que el engranaje use el acento guardado
                >
