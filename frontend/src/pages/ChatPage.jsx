...
function ChatPage() {
  ...
  const { themeColor, receiverColor, isTextBold, chatFontSize } = useConfigStore();
  ...
  <div
    className="w-full h-screen bg-black absolute inset-0 overflow-hidden text-slate-200"
      // INYECCIÓN BLINDADA CON FALLBACKS DIRECTOS
      style={{
  ...
  <div className="absolute bottom-4 left-4 z-30 flex flex-col items-center group">
    ...
    <span 
      ...
      style={{ 
        ...
        borderColor: "rgba(var(--theme-primary), 0.3)" // Opcional: si querés que el borde del tooltip también use el color acento
      }}
    >
    ...
    <button
      onClick={() => navigate("/config")} // ← Corregido a "/configuracion" para que coincida con App.jsx
      className="p-3 bg-slate-800/80 text-slate-400 rounded-xl border shadow-xl backdrop-blur-md transition-all duration-300 transform hover:scale-110 hover:animate-[spin_4s_linear_infinite]"
      style={{
        // Usamos estilos en línea o la clase de Tailwind arbitraria hover:text-primary 
        // pero inyectando directamente el color dinámico para asegurar la reactividad:
      }}
      // Para no perder la comodidad de Tailwind con el hover dinámico, podemos usar clases arbitrarias:
      className="p-3 bg-slate-800/80 text-slate-400 hover:text-primary border border-slate-700/50 hover:border-primary/40 shadow-xl backdrop-blur-md transition-all duration-300 transform hover:scale-110 hover:animate-[spin_4s_linear_infinite]"
